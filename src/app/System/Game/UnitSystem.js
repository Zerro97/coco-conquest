...
export class UnitSystem extends System {
  ...
  updateMovementRange() {
    ...
    // Only update movement range array when unit is selected
    // and movement range isn't updated
    if(selectedUnit && (Object.keys(weightMap.value).length === 0 || 
          ...
          for(let x=-1; x <= 1; x++) {
            for(let y = Math.max(-1, -x-1); y <= Math.min(1, -x+1); y++){
              ...
              if(!(x === 0 && y === 0 && z === 0) &&
                ...
                if (curWeight < weightMap.value[node.x + x][node.y + y][node.z + z] && curWeight <= speed) {
                  weightMap.value[node.x + x][node.y + y][node.z + z] = curWeight;
  ...
  moveUnit() {
    ...
    if(selectedUnit && movingTile) {
      ...
      if(weightMap.value[movingPos.x] &&
